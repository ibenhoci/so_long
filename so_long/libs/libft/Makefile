# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: ibenhoci <ibenhoci@student.42.fr>          +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2023/04/08 19:51:48 by ibenhoci          #+#    #+#              #
#    Updated: 2023/04/13 14:37:42 by ibenhoci         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME = libft.a

SOURCES = ft_isalnum.c ft_atoi.c ft_bzero.c ft_calloc.c  \
ft_isalpha.c ft_isascii.c ft_isdigit.c ft_isprint.c  \
ft_memchr.c ft_memcmp.c ft_memcpy.c ft_memmove.c  \
ft_memset.c ft_putchar_fd.c ft_putendl_fd.c ft_putnbr_fd.c  \
ft_putstr_fd.c ft_strchr.c ft_strdup.c ft_strjoin.c  \
ft_strlcat.c ft_strlcpy.c ft_strlen.c ft_strncmp.c  \
ft_strnstr.c ft_strrchr.c ft_substr.c ft_tolower.c  \
ft_toupper.c ft_itoa.c ft_strmapi.c ft_striteri.c \
ft_strtrim.c ft_split.c

BONUS = ft_lstnew.c ft_lstsize.c ft_lstlast.c ft_lstadd_front.c \
ft_lstadd_back.c ft_lstdelone.c ft_lstclear.c ft_lstiter.c \

FLAGS = -Wall -Wextra -Werror
CC = cc -c

all: $(NAME)
$(NAME):
		$(CC) $(FLAGS) $(SOURCES)
		ar rcs $(NAME) *.o

bonus:
		$(CC) $(FLAGS) $(BONUS)
		ar rcs $(NAME) *.o

clean:
		rm -f *.o

fclean: clean
		rm -f $(NAME)

re: fclean all

.PHONY: all clean fclean re bonus
